...
class MenuCharacter extends FlxSprite
{
	private static var settings:Map<String, CharacterSetting> = [
		...
		'cuphead' => new CharacterSetting(0, 0, 1.0)
	];
	...
	public function new(x:Int, y:Int, scale:Float, flipped:Bool)
	{
		...
		animation.addByPrefix('cuphead', "Cuphead", 24, false);
		//animation.addByPrefix('cupheadConfirm', 'Cuphead enter', 24, false);
	...
	public function bopHead(LastFrame:Bool = false):Void
	{
		if (character == 'gf') {
			...
			//no spooky nor girlfriend so we do da normal animation -kade
			//and also kade is an idiot -shadow
			//:O dude thats sick -101% not shadow
			if (animation.name == character + "Confirm")
				return;
